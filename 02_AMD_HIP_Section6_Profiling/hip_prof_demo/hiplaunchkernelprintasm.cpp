...
int main() {
    ...
    // 3. Compile the program for Execution
    std::string arch_flag = "--gpu-architecture=" + std::string(prop.gcnArchName);
    const char* opts[] = { arch_flag.c_str() }; // Standard compilation for Code Object
    
    hiprtcResult compile_result = hiprtcCompileProgram(prog, 1, opts);
    
    // 4. Get compilation log (always print for debugging)
    size_t log_size;
    ...
    // 5. Get the compiled code (ISA Code Object)
    size_t code_size;
    HIPRTC_CHECK(hiprtcGetCodeSize(prog, &code_size));
    std::cout << "Computed code size: " << code_size << std::endl;

    if (code_size == 0) {
        std::cerr << "Error: Code size is 0. Compilation might have failed to produce an object." << std::endl;
        exit(1);
    }
    ...
    // 6. Destroy the main program
    HIPRTC_CHECK(hiprtcDestroyProgram(&prog));

    // 7. Introspection: Separate compilation for LLVM Bitcode (requires -fgpu-rdc)
    {
        std::cout << "Compiling for LLVM Bitcode introspection..." << std::endl;
        ...
        if (hiprtcCompileProgram(prog_ir, 2, opts_ir) == HIPRTC_SUCCESS) {
            size_t bitcode_size;
            ...
            std::system("cat dump_kernel.ll");
             ...
             std::cout << "Skipping bitcode dump (compilation failed for IR pass)" << std::endl;
        }
