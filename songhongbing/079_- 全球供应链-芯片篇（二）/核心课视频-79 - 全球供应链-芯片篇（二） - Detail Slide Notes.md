# 核心课视频-79 - 全球供应链-芯片篇（二） — Detail Slide Notes
...
這張圖高度概括了EDA的核心原理這個EDA我們一般很多人談的EDA是從技術角度去談但是我在研究EDA的時候我更喜歡用哲學視角去看就是我們以前讀過了黑耳的小邏輯黑耳認為哲學的本質是邏輯自然科學他是認識自...
...
> 的系統比如華為、海斯它選擇Arm這個指令級系統之後它才能去設計它自己的麒麟芯片的處理器系統那麼與此相對應它才能夠進行芯片的空間的分層物理分層
> 、物理分區還有它的功能分區等等才能進行這些設計那麼從行為角度來看確定了這個算法指令之後然後就要用這個就要進行所謂基存器傳輸級的語言編程用手工
> 編程語言來描述硬件的功能所謂基存器傳輸級我們可以用軟件的概念可以很容易理解它也就是說它是一個程序中間的全局變量你在全局變量中定義、
> 影響整個程序前後左右所有功能的總的變量都在那時限所以基存器級、基存器傳輸級相對應軟件的就是這個變量、
> 全局變量這部分是用手工的編程語言來描述的硬件描述語言然後有了這些功能的功能誇二的話分之後就進入邏輯韓述就是你這個功能誇二的話誇二的話誇二了每
> 個功能誇二要實現什麼樣的邏輯啊最後這個通過與非門這個最後出來是什麼東西啊就要設計到邏輯韓述就到這了從算法一繞基存器傳輸級一直到這個邏輯、
> 邏輯韓述然後再到微分方程也就是這個電路出來收到刺激之後它的響應的區限怎麼來用數學來求解怎麼來求解然後最後求解完了之後我是要用訪針來驗證的就是
> 你設計籃圖是這樣的一個區限最後在電路中呈現這樣一個區限我能不能夠訪針出來訪針就是說我還沒有真正做芯片我還沒有物質上的那個電路存在但是由於我知
> 道你的微分方程是這麼描述這個區限的我就用一個純粹虛擬的微分方程這樣一個模塊來替代實現在我沒有具體電路之前我可以用算法把這個區限模擬得很好我看
> 你跟我這個最後我這算法模擬出的區限是不是一致不就完了嗎這就叫訪針這就是在我設計和生產芯片在流片之前我就已經完成了這樣一個功能上的檢測最後你生
> 產之後芯片當然錯誤的概率就很小了假如我沒有這個這部描述我沒有經營訪針那就聽天有命了那出來一次流片就損失幾十萬幾百萬幾千萬都有可能那沒有一家公
> 司能承受起這麼高的一個錯誤率那整個公司會破產理解了1.3個維度理解它的核心之後我們再來看1.1的進化路徑好看下一頁1.1進化了三個階段第一階段最核心的一環路
...
在這個階段完成的時代是六七十年代六十到七十年代首先是CAD出現了這是六十年代中出現了CAD計算機輔助設計工具CAD工具不僅用於機械設計也被用於繪製電路圖它是中間的很多多邊形比如走線比如電器原件之類的那...
...
> 了邏輯訪針器和電路訪針器加在一起一塊形成了自動化程度更高的訪針與驗證就是底層是這個電路訪針器在直上是邏輯訪針器兩個結合在一起就會實現強大的自
> 動化那麼總結一下第二代EDA處在二環路也就是邏輯韓數門電路和單元佈置幾顆圖星從幾顆角度來說就單元佈置這樣一個二環之內實現了電路在邏輯層和在邏
> 輯層的這種自動的訪針和功能的驗證極大提升了設計效率而底層的佈線還有電路訪針都已經完全自動化了第三代EDA當然是從80年代到現在它的突破性的進
> 展是填在這篇論文超大規模競爭電路島論系統島論這是1980年米德和康維聯合發表的在這篇論文中間第一次提出了通過編程語言來進行芯片設計就是硬件描
> 述語言HDL出現了兩個兩種語言這兩種語言的出現使得設計者可以在更高層的這種抽象層就是基層器傳輸層傳輸級ATL這個層次上來直接描述電路的功能我
> 在這兒總結了一句以編程語言來設計硬件功能這是EDA進化始終最具革命性的突破這是一次革命性的變化這些東西都還是次要的雖然工具作用的話很重要電路
> 訪針節底層的工具也很重要但是用HDL來直接描述硬件用程序語言自動來描述硬件的功能這是翻天復底的變化我可以用編程來實現這一點它的重要作用就是整
> 個芯片設計未來它的發展方向變成了分層化標準化、模塊化、成就化和自動化當然你就說我們看到用這種高層語言來描述電路的功能描述完了怎麼辦呢描述完了
> 接上下來叫靠邏輯綜合工具把這種高層描述的程序語言自動翻譯為門機網表我們知道門機網表稍在80年代已經實現了就是在往下已經完全自動化了門機網表和
> 經濟管集的網表能夠自動完成所有經濟管集別的所有電路的佈現這在70年代、80年代都已經準備好的工具了邏輯綜合能夠通過H掉這種高層語言直接掉底下
> 讓所有下面的東西自動事件當然除此之外還有邏輯形式驗證使用布爾代出工具來驗證門機電路的行為那麼這種驗證它要看它是不是與高層設計那種語言描述的行
> 為相一致然後來確保邏輯綜合的準確性就是行事驗證是來反過來來教驗邏輯綜合你到底綜合對沒有啊你這個翻譯到底翻譯合理不合理啊我得經驗證除此之外還有
> 淨態失去分析我們知道當電流在電路中走的時候它一定會有延遲各種路徑上的這個門機電路都會有產生不同的延遲那麼我們知道計算機和這種數字電路你都是靠
> 一個始終賣充那麼在一個始終賣充之內你要完成很多指令這個始終賣充起來這個端那麼它在門機電路中會出現延遲這個延遲如果延遲到非常糟糕的程度的話會是
> 整個的這個電路功能出問題所以淨態失去分析必須要計算和累加所有可能路徑上的門機電路的延遲加在一起到底延遲多少是不是超過一個始終賣充的這樣一個這
> 樣一個遲度和界線超過了那就出問題了它用於確保電路功能在一個始終賣充之內它的完整性和可靠性就是要確保它能夠完成總結起來第三代已經擴展到了三環路
> 就是ATL就是計存器傳輸機的這種高級語言和功能模塊包括模塊的佈置這形成了三環路從此以後設計者、
> 芯片設計者只需要專注於功能邏輯而不必再分心於底層怎麼實現了怎麼佈現不用去考慮了這都自動化了EDA設計效率進步最終使得芯片設計能夠追得上模二定
> 律這是靠EDA的工具來實現的這就是為什麼EDA非常了不起沒有這套東西就根本談不上芯片設計你在牛的芯片公司沒工具怎麼設計那千別拿尺子這是不可能
> 設計芯片的這就是我們很多人其實對EDA的邏輯它背後的邏輯和底層的原理搞清楚所以你不知道這個工具的強大和它的基礎性至關重要好我們再看下一頁那麼美國EDA我們剛剛說了三句頭四句頭還是五句頭
...
定義不一樣但是最主要是三大霸主這三大霸主其實從八十年代再往前和再往後基本上都是三家獨大的局面這可能有一個穩定性還是一個什麼樣的原理三家支撐這個市場那麼2020年美國EDA的三句頭新自科技凱晨戀子還有新...
...
> 主要是要它的工具要工具背後使用的客戶然後要市場份額然後把這個吞麵來公司迅速整個到它的1D的工具鏈上然後大量收購80多次收購它的工具鏈就會越來
> 越齊全承登和新聞子分別收購了62次和66次所以我們看到巨頭的起來收購監病很重要但是前提是內成研發沒有內成研發外向監病一定會失敗因為你都搞不清
> 楚你核心產品是什麼你怎麼去整合別人亂七八道東西往哪整合一得首先要自己內部的一個非常強大的一個平台才能夠有效整合外部的要素所以首先是內循環轉通
> 了之後外循環才能搭配內循環產生更大效率好我們再看下一頁陸近依賴1D首先跟代工廠形成了工具思索
...
這是一個新片設計的一個邏輯框圖1D工具供應商在中間代工廠在這我們看到1D首先可以代工廠提供各種開發工具讓它在生產新片這個代工廠它也需要1D工具來幫助它來設計一些基本的標準庫同時還給它提供很多建模的工具...
...
> 完之後我在搞我的然後穿在一起需要三年時間現在兩者是一起研發一起起步一起在一起工作大家在一起工作共同設計所以比以前捷約了一半的時間成為也大大下
> 降與此同時台積電和新司科技都不用再重複投資各自結成了時間接上金錢當然同時就把台積電跟EDA徹底綁死了當然中心國際中國的大陸這些代工廠這個模式
> 也一樣跟台積電沒有兩樣他也得跟EDA公司聯手才能夠才能夠把新的製成這個工藝才能變成實實在在的能夠讓新片設計公司可以用的東西比如說你要向他們提
> 供PDK第二步代工廠向新片設計公司提供PDK這個PDK就是新片設計工具包其中包含了各種標準單元庫還有頻繁更新的基本基本特徵信息庫這將使新片設
> 計公司得以利用最先進的製成的性能這些PDK中間欠入了或者叫直入了EDA的模塊所以進一步實現了新片設計公司和EDA的死所由於代工廠台積電這樣的
> 公司中心國際這樣的公司它的PDK中間含有美國的EDA技術包括基本特徵信息庫裡面也有EDA的技術所以到美國制裁華為的時候台積電和中心國際都沒有
> 辦法向華為提供新片這個就是一個細節我們很多人不清楚覺得中心國際是中國的為什麼你不向華為提供新片它受美國的制約就是你得用美國的EDAEDA生產
> 的工具而且你開發了PDK中間也有EDA的基因美國EDA的基因所以你受美國技術的控制第三步EDA向新片設計公司提供設計訪針和驗證的工具當然這就
> 是新片設計公司只有依賴EDA的工具才能進行設計第四新片設計公司向代工廠提交設計數據文件就是GDS2這就是最後的一個數據文件有了GDS2這個文
> 件之後代工廠拿到這個數據文件它就可以進行光眼磨然後就可以進行光顆機就開始生產知道了這個大概就是整個新片生態EDA生態的一個完整的框圖但這個就
> 是屬於現在多新片的情況的設計了就是設計到一種先進的這樣一種SIP這樣的技術了後模二時代模二時代到兩三納米的時候再往後就面臨物理機械了怎麼辦所
> 以新片設計開始很多新型技術比如說堆疊堆疊成很多層比如說CipulateCipulate也是大量利用把不同的功能磨塊不同功能磨塊的新片組合在一
> 起都是同樣意思就是要通過空間通過不同的新片之間的組合包裝來達到同樣的目的因為新片在往下走模二定律已經走不下去了這就睡到外包的瘋裝測試就是我可
> 以把瘋裝測試這塊外包出去這就是這位框圖所形成的這樣一個流程EDA的巨頭現在也已經切入進來了他們搞了ADK就給包裝公司聽幫他們研發ADKADK
> 再給IC的這個新片設計公司所以又形成了一個新的思索ADK不管是在傳統的新片設計領域中還是在現在新新的這種外包瘋裝測試領域中都有自己的DNA這
> 就導致這兩種模式誰也離不開美國的巨頭我們再看下一頁還有一個優勢就是平台依賴
...
主要是跟新片設計特別是IP思索在一起我們來看全世界IP市場就是設計中間的重新發明輪子不用重新發明輪子那幫人搞了IP的IP包或者IP核它的市場分布情況主要CPU為主這個中央處理器這套系統為主但還有數字信...
...
> 一來EDA的IP庫就會變得越來越豐富有它自己的也有別人的那它的規模越來越大它的規模越來越大新片設計公司就更加依賴EDA從而使得更多的IP供應
> 商願意把自己產品放到EDA的平台上來積極交易最終實現了EDA對第三方IP核的平台思索這就是為什麼美國三大巨頭一旦控制平台控制生態之後基本上形成了一個徹底思索的這種關係那麼第四代EDA錄在核方這是夏業
...
第四代EDA會向哪個方向發展我覺得現在已經可以看出一些瞄頭來了我們看左邊這張圖這張圖講的是未來2020年的2027年整個EDA市場發展的規模從現在的106億發展到未來的167億美元嚴峻增長速度符合增長...
...
雖然中國的EDA市場規模增長很快從2015年到現在每年都在快速增長2020年我們看到中國EDA市場的規模達到了93.1億億人民幣當然在這個整個的銷售中間大部分絕大部分是美國EDA巨頭創造出來了銷售值所...
...
> 字芯片模擬壹變沒有沒有怎麼太去撲這個市場所以華大九天擁有芯片就是模擬芯片和數模混合芯片的這種全流成工具這也是全世界大概只有三次加有華大九天算
> 壹加在模擬新聞中這家公司華大九天能夠提供全流成的就像美國新斯科技在數字芯片裏中所佔到了一個地位特別是電路訪真模擬工具的這種性能非常有色這種性
> 能非常出色在國際市場競爭力很強另外就是它把這套東西用於顯示板就是那種業經顯示領域中形成了全鏈條的工具全世界好像只有華大九天擁有全流成的這種業
> 經顯示板的設計的全流成工具它在這個方面因為很多像全球獨特的領先的技術好 我們再看下頁
